    ...
    return (
        <div className=' py-10'>
            <div className=' min-w-full flex flex-row gap-8 md:gap-16 bg-slate-200 justify-center items-center text-xl font-bold py-3 rounded-md'>
                <button onClick={() => setTabs("description")} className={`${tabs === "description" && "text-red-500"}`}>Add to card</button>
                <button onClick={() => setTabs("review")} className={`${tabs === "review" && "text-red-500"}`}>Review</button>
            </div>
            {tabs === "review" ? (
